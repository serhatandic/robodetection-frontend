...
function App() {
	...
	useEffect(() => {
		const handleConnection = async () => {
			try {
				const resp = await fetch('http://144.122.71.16:8080/', {
					method:'GET',
					})
					if (resp.status === 200){
	...
	return (
		<div className='h-screen w-screen p-4 flex justify-center'>
			{connectionStatus === 'connecting' ? <CircularProgress  className='m-auto'/> : <>
			...
			>
				<Box className='h-20 bg-white rounded-md p-5 flex flex-col justify-center items-center'>
					<Typography variant="h6" component="h2">
						Connection could not be established, you will not be able to use the controllers
					</Typography>
					<Button variant='contained' className='w-1/4' onClick={() => {
						setModalStatus(false)
					}}>
						Continue
					</Button>
